...
typedef enum cham_t_callback_types_t {
    ...
    cham_t_callback_task_processed              = 14
...
typedef struct cham_t_task_list_t {
    ...
    void set_end_time(TYPE_TASK_ID task_id, double end_time){
        this->m.lock();
        for (std::list<cham_t_task_info_t *>::iterator it=this->task_list.begin(); it!=this->task_list.end(); ++it){
            if ((*it)->task_id == task_id){
...
// push some noise-time into the execution function
...
// callback gets start_time when a task is processed
typedef void (*cham_t_callback_task_processed_t)(
    cham_migratable_task_t * task
);

// get end_time of each task
typedef void (*cham_t_callback_task_end_t)(
    cham_migratable_task_t * task
);

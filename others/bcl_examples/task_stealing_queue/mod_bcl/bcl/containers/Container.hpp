...
namespace BCL {
  ...
  template <size_t N>
  struct serialize <task, N, void> {
    serial_blob <double, N> operator()(const task &t) const noexcept {
      serial_blob <double, N> blob;

      for (int i = 0; i < t.matrixSize * t.matrixSize; i++) {
        blob.val[i] = t.matrix[i];
      }
      for (int i = t.matrixSize*t.matrixSize; i < 2*t.matrixSize * t.matrixSize; i++) {
        blob.val[i] = t.matrix2[i-t.matrixSize*t.matrixSize];
      }
      blob.len = t.matrixSize*t.matrixSize*2;
      return blob;
    }
    task deserialize(const serial_blob <double, N> &blob) const noexcept {
      task t;
      t.matrixSize = sqrt(blob.len/2);
      t.matrix = (double*)malloc(sizeof(double) * t.matrixSize * t.matrixSize);
      t.matrix2 = (double*)malloc(sizeof(double) * t.matrixSize * t.matrixSize);
      // printf("[%ld]Alloced %d\n", BCL::rank(), t.matrixSize*t.matrixSize*2);
      
      for(int i = 0; i<t.matrixSize*t.matrixSize; i++){
        t.matrix[i] = blob.val[i];
        // printf("Matrix[%d]: %lf\n",i,t.matrix[i]);
      }
      for(int i = 0; i<t.matrixSize*t.matrixSize; i++){
        t.matrix2[i] = blob.val[i+t.matrixSize*t.matrixSize];
      }
      return t;
    }
  };

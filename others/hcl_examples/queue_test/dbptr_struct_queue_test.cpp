...
int main (int argc, char *argv[])
{
    ...
    hcl::queue<double_ptr_t> *global_queue;

    // allocate the hcl queue at server-side
    if (is_server) {
        global_queue = new hcl::queue<double_ptr_t>();
    }
    MPI_Barrier(MPI_COMM_WORLD);
    ...
    std::queue<double_ptr_t> local_queue;
    ...
    // check task size
    int mat_size = 10;
    double_ptr_t tmp_T = double_ptr_t(mat_size);
    ...
    if (!is_server) {
        ...
        for(int i = 0; i < num_tasks; i++){
            ...
            double_ptr_t lT= double_ptr_t(mat_size);
    ...
    if (!is_server) {
        ...
        for(int i = 0; i < num_tasks; i++){
            // allocate the task
            double_ptr_t gT = double_ptr_t(mat_size);
